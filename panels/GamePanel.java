...
import matter.Player;
import settings.Settings;
import tools.Camera;
import tools.FileRead;
import tools.ObjectDimensions;

public class GamePanel extends MasterPanel implements KeyListener, Runnable {
	...
	private final int JUMPHEIGHT = (int) (3 * BLOCKSIZE);
	...
	public GamePanel() {
		...
		beatLevelButton.setText(
				"<html><center>" + "You beat the level!" + "<br>" + "Click here to continue." + "</center></html>");
		...
		lostLevelButton.setText(
				"<html><center>" + "You lost the level." + "<br>" + "Click here to continue." + "</center></html>");
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				...
				if (runTimer) {
					...
					secTime = TimeUnit.MILLISECONDS.toSeconds(totalTime)
	...
	public void keyPressed(KeyEvent ke) {
		...
		if ((key.equals(KeyEvent.VK_SPACE) || key.equals(KeyEvent.VK_UP) || key.equals(KeyEvent.VK_W))
				&& !spacePressed) {
			// space bar/up/w
			spacePressed = true;
		}
